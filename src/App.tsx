...
const extractTagContent = (plan: string, tag: string): string => {
  ...
  // Map internal tags to actual API tags
  ...
  // Get all possible tag names for this section
  const possibleTags = tagMap[tag] || [tag];
  
  // Try each possible tag
  ...
  // If no tag found, try to find by keywords
  const keywords: Record<string, string[]> = {
    'SEGMENTATION OUTPUT': ['segmentation', 'segment', 'market segment', 'target segment', 'primary target'],
    'TAMSAMSOM OUTPUT': ['tam', 'sam', 'som', 'market size', 'addressable market', 'total addressable'],
    'KPI OUTPUT': ['kpi', 'key performance', 'metric', 'measurement', 'performance indicator', 'primary kpi'],
    'OKRS OUTPUT': ['okr', 'objective', 'key result', 'objective and key result', 'okrs'],
    'PESTLE OUTPUT': ['pestle', 'political', 'economic', 'social', 'technological', 'legal', 'environmental', 'drivers'],
    'PORTERS OUTPUT': ['porter', 'five forces', 'competitive force', 'industry rivalry', 'porter\'s', 'threat of new entrants', 'bargaining power'],
    'COMPETITOR OUTPUT': ['competitor', 'competition', 'competitive', 'rival', 'competitors', 'competitive advantage', 'threat level'],
    'POSITIONING OUTPUT': ['positioning', 'position', 'brand position', 'value proposition', 'brand statement', 'core value'],
    ...
    'JOURNEY OUTPUT': ['journey', 'customer journey', 'buyer journey', 'touchpoint', 'customer journey map', 'awareness', 'consideration', 'purchase', 'retention', 'advocacy'],
    'ROADMAP OUTPUT': ['roadmap', 'timeline', 'milestone', 'phase', 'step', 'road map', 'phase 1', 'phase 2', 'days']
  };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (found) {
      ...
      if (lines[i].match(/^\[/) || lines[i].match(/^---/) || lines[i].match(/^\*\*/)) {
        break;
      }
      // Stop if we hit another keyword
      let stop = false;
      for (const kw of keywordList) {
        if (lines[i].toLowerCase().includes(kw) && i > foundLineIndex + 1) {
          stop = true;
          break;
        }
      }
...
const SegmentationVisual = ({ plan }: { plan: string }) => {
  ...
  const parseSegments = (): Segment[] => {
    ...
    if (content) {
      ...
      for (const line of lines) {
        ...
        // Look for segment patterns like "Primary Target Segment: ..." or "1. ..."
  ...
  const getPrimaryTarget = (): string => {
    ...
    const match2 = plan.match(/primary target[:\s]*([^\n]+)/i);
    ...
    if (tableRows.length > 0) {
      for (const row of tableRows) {
        ...
        competitors.push({
          ...
          threat: threat,
          offering: row[1]?.trim().substring(0, 40) || 'Competitor offering',
